...
# Add more shuffles for ROIs with low p-values: (Again)
...
for roi in tqdm.tqdm(glob.glob(newsavedir+'*.h5')):
	...
	for task,taskv in roidict.items():
		...
		if 'auc_diff' in taskv.keys():
			nshuff = len(taskv['auc_diff']) - 1
			if nshuff != len(taskv['ll_diff']) - 1:
				print('ERROR\nroi:',roi,': Number of shuffles for auc != ll!!')
				break
			p_ll_ = np.sum(abs(taskv['ll_diff'][0])<abs(taskv['ll_diff'][1:]))/nshuff
			p_auc = np.sum(abs(taskv['auc_diff'][0])<abs(taskv['auc_diff'][1:]))/nshuff
			if p_ll_<0.05 or p_auc<0.05:
				...
				best_k = taskv['best_k']
				tune_seg = np.append(taskv['tune_seg_perm'], np.zeros((nshuff2,nbins,nsplit,nTR_,best_k)), axis=0)
				tune_ll = np.append(taskv['tune_ll_perm'], np.zeros((nshuff2,nbins,nsplit)), axis=0)
				for split,Ls in enumerate(kf.split(np.arange(nsub),y)):
					...
					hmm.fit(Dtrain)
					for shuff in np.arange(nshuff+1,nshuff+nshuff2+1):
						# RANDOMIZE
				...
				roidict[task]['E_k'] = np.zeros((nshuff+nshuff2+1,nbins,nTR_))
				roidict[task]['auc'] = np.zeros((nshuff+nshuff2+1,nbins))
				roidict[task]['auc_diff'] = np.zeros(nshuff+nshuff2+1)
				roidict[task]['ll_diff'] = np.zeros(nshuff+nshuff2+1)
				for shuff in range(nshuff+nshuff2+1):
					for bi in range(nbins):
		...
		elif p_ll_<0.05 or p_auc<0.05:
			print('ERROR\nroi:',roi,'needs more shuffles, go back to 8_HMM_stats_p.py!!')
			break

...
ROIfold = path+'ROIs/SfN_2019/ROIs_Fig3/Fig3_' #path+'ROIs/g_diff/'
HMMf = HMMpath+'HMM_'+str(date.today())+'.h5'
#HMMf = HMMpath+'HMM_2019-09-26.h5'
	...
	for f in tqdm.tqdm(glob.glob(ROIfold+'*roi')):
		...
		if roin not in ROIs.keys():
					...
					for k in k_list:
						...
						for w in win_range: # windows in range 5 - 10 sec
							...
							for p in range(nshuff):
								...
								grpp.create_dataset('within_r_'+str(p),
													data=corrs[rand_events[:-w] == rand_events[w:]])
								grpp.create_dataset('across_r_'+str(p),
													data=corrs[rand_events[:-w] != rand_events[w:]])
			...
			for b in bins: # error on next line
				...
				for split in range(nsplit):
					ltxt = '/'+roin+'/bin_'+str(b)+'/split_'+str(split)
					# test on same subset as above
					Dtest = D[dd.io.load(HMMf,ltxt+'/LO')]
					grps = grpb.require_group('split_'+str(split))
					for k in k_list:
						ltxt = '/'+roin+'/bin_'+str(bo)+'/split_'+str(split)
						grpk = grps.require_group('k_'+str(k))
						pat = dd.io.load(HMMf,'/'+roin+'/bin_'+str(bo)+'/split_'+str(split)+'/k_'+str(k)+ '/pattern')
						var = dd.io.load(HMMf,'/'+roin+'/bin_'+str(bo)+'/split_'+str(split)+'/k_'+str(k)+'/event_var')
						hmm_perm = brainiak.eventseg.event.EventSegment(n_events=k)
		...
		for b in bins:
			bin = 'bin_'+str(b)
			grpb = grpf.require_group(bin)
			grpa = grpb.require_group('all_sub_events')
			D = dd.io.load(HMMf,'/'+roin+'/'+bin+'/D')
			# set final patterns and event timings for each k with all subjects:
			for k in k_list:
				grpk = grpa.require_group('k_'+str(k))
				#fit HMM
				hmms_wb = brainiak.eventseg.event.EventSegment(n_events=k)
				hmms_wb.fit(np.mean(D,axis=0).T)
				if ROIs:
					datstr = '/'+roin+'/'+bin+'/all_sub_events/k_'+str(k)+'/'
					del hf[datstr+'pattern']
					del hf[datstr+'seg_og']
					del hf[datstr+'event_var']
				grpk.create_dataset('pattern',data=hmms_wb.event_pat_)
				grpk.create_dataset('seg_og',data=hmms_wb.segments_[0])
				grpk.create_dataset('event_var',data=hmms_wb.event_var_)

...
subla = {key:{key:{key:[] for key in range(nbinseq)} for key in range(nsh)} for key in [True,False]}
D = {key:{} for key in range(nbinseq)}
ISC_w_time = np.zeros((2,nsh,nbinseq,n_time))
ISC_w = np.zeros((2,nsh,nbinseq))
ISC_b_time = {{key:{} for key in range(nsh)} for key in [True,False]}
ISC_b = {{key:{} for key in range(nsh)} for key in [True,False]}
ISC_g_time = {{key:{} for key in range(nsh)} for key in [True,False]}
gdict = {{key:{'Age1':[],'Age2':[],'g_diff':[]} for key in range(nsh)} for key in [True,False]}

for boot in [True,False]:
	bootidx = 0 if True else 1
	print('Bootstrapping') if boot == True else print('Not bootstrapping')
	agel = agelperm
	phenol = phenolperm
	for s in tqdm.tqdm(range(nsh)): # Either permuted shuffle or bootstrapped resample iterations
		...
		for b in range(nbinseq):
			...
			for sub in subl[0]+subl[1]:
				if boot == True and sub not in D[b]:
					D[b][sub] = np.mean(zscore(dd.io.load(sub,['/'+task+'/HPC'])[0],axis=1),axis=0)
				elif boot == False and all(sub not in D[b_] for b_ in range(nbinseq)):
			...
			ISC_w[bootidx,s,b] = np.sum(ISC_w_time[s,b])/(n_time-1)
		for p in itertools.combinations(range(nbinseq),2):
			p_str = str(p[0])+'_'+str(p[1])
			ISC_g_time[boot][s][p_str] = []
			ISC_b_time[boot][s][p_str] = []
			for htmp1 in [0,1]:
				for htmp2 in [0,1]:
					ISC_b_time[s][p_str].append(np.multiply(groups[p[0],htmp1], groups[p[1],htmp2]))
			ISC_b[boot][s][p_str] = [np.sum(t)/(n_time-1) for t in ISC_b_time[s][p_str]] # correlate across bins
			ISCg_time = np.sum(ISC_b_time[s][p_str],axis=0)	
			ISCg = np.sum(ISC_b[s][p_str])	
			ISC_g_time[boot][s][p_str] = ISCg_time/4/(np.sqrt(ISC_w[s,p[0]])*np.sqrt(ISC_w[s,p[1]]))
			...
			for k in gdict[s].keys():
				...
				if 'Age' in k:
					...
					gdict[boot][s]['g_diff'].extend([ISCg])
		if boot == False: # randomly shuffle ages:
...
dd.io.save(ISCpath+'HPC.h5',{'D':D,'subla':subla, 'ISC_w_time':ISC_w_time, 'ISC_w':ISC_w, 'ISC_b_time':ISC_b_time, 'ISC_b':ISC_b, 'ISC_g_time':ISC_g_time, 'ISC_g_time_2':ISC_g_time_2, 'gdict':gdict})
...
sns.swarmplot(x='Age',y='ISC',data=df,zorder=1)
...
for event in event_list:
	...
	for b in range(nbinseq):
		for s in range(nsh):
			...
			ISCedict['ISC'].extend(ISC_w_time[s,b,event-TW//2:event+TW//2])
...
for event in event_list:
	...
	for p in itertools.combinations(range(nbinseq),2):
		for s in range(nsh):
			...
			ISCgdict['ISC'].extend(ISC_g_time[s][str(p[0])+'_'+str(p[1])][event-TW//2:event+TW//2])
...
for p in itertools.combinations(range(nbinseq),2):
	if max(range(nbinseq)) in p:
		for s in range(nsh):
			...
			ISCgtimedict['ISC'].extend(ISC_g_time[s][str(p[0])+'_'+str(p[1])])
			ISCgtimedict['z-scored ISC'].extend(zscore(ISC_g_time[s][str(p[0])+'_'+str(p[1])]))
...
for b in range(nbinseq):
	for s in range(nsh):
		...
		ISCetimedict['ISC'].extend(ISC_w_time[s,b])
		ISCetimedict['z-scored ISC'].extend(zscore(ISC_w_time[s,b]))
...
for p1 in itertools.combinations(range(nbinseq),2):
	if max(range(nbinseq)) in p1 and p1!=p2:
		for s in range(nsh):
			ISCp1 = ISC_g_time[s][str(p1[0])+'_'+str(p1[1])]
			ISCp2 = ISC_g_time[s][str(p2[0])+'_'+str(p2[1])]
...
for b1 in range(nbinseq-1):
	for s in range(nsh):
		xcorr = np.correlate(ISC_w_time[s,b1],ISC_w_time[s,b2],"full")
